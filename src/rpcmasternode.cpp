...
Value privatesend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "privatesend <epraddress> <amount>\n"
            "epraddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "privatesend <epraddress> <amount>\n"
            "epraddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) epr address of current masternode\n"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in epr.conf\n"
...
Value listmasternodes(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode epr address\n"
...
Value masternodedebug (const Array& params, bool fHelp)
{
    ...
    if (activeMasternode.status != ACTIVE_KARMANODE_INITIAL || !masternodeSync.IsSynced())
...
Value startmasternode (const Array& params, bool fHelp)
{
    ...
    if (strCommand == "local") {
        ...
        if (activeMasternode.status != ACTIVE_KARMANODE_STARTED) {
            activeMasternode.status = ACTIVE_KARMANODE_INITIAL; // TODO: consider better way
    ...
    if (strCommand == "all" || strCommand == "many" || strCommand == "missing" || strCommand == "disabled") {
        ...
        if ((strCommand == "missing" || strCommand == "disabled") &&
            (masternodeSync.RequestedMasternodeAssets <= KARMANODE_SYNC_LIST ||
                masternodeSync.RequestedMasternodeAssets == KARMANODE_SYNC_FAILED)) {
...
Value getmasternodestatus (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) epr address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
Value getmasternodewinners (const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) epr MN Address\n"
