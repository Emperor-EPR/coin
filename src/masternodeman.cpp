...
void CMasternodeMan::CheckAndRemove(bool forceExpiredRemoval)
{
    ...
    while (it != vMasternodes.end()) {
        if ((*it).activeState == CMasternode::KARMANODE_REMOVE ||
            (*it).activeState == CMasternode::KARMANODE_VIN_SPENT ||
            (forceExpiredRemoval && (*it).activeState == CMasternode::KARMANODE_EXPIRED) ||
    ...
    BOOST_FOREACH (CMasternode& mn, vMasternodes) {
        ...
        if (IsSporkActive (SPORK_8_KARMANODE_PAYMENT_ENFORCEMENT)) {
            ...
            if ((nMasternode_Age) < nMasternode_Min_Age) {
                continue; // Skip masternodes younger than (default) 8000 sec (MUST be > KARMANODE_REMOVAL_SECONDS)
            }
        }
        mn.Check ();
        if (!mn.IsEnabled ())
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    // Light version for OLD MASSTERNODES - fake pings, no self-activation
        ...
        if (fAcceptable) {
            ...
            // should be at least not earlier than block when 1000 EPR tx got KARMANODE_MIN_CONFIRMATIONS
            uint256 hashBlock = 0;
            CTransaction tx2;
            GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
            BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
            if (mi != mapBlockIndex.end() && (*mi).second) {
                CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 10000 EPR tx -> 1 confirmation
