...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::EPR);
    ...
    if (!settings.contains("nAnonymizeEPRAmount"))
        settings.setValue("nAnonymizeEPRAmount", 1000);

    nPrivateSendRounds = settings.value("nPrivateSendRounds").toLongLong();
    nAnonymizeEPRAmount = settings.value("nAnonymizeEPRAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeEPRAmount"))
        SoftSetArg("-anonymizeepramount", settings.value("nAnonymizeEPRAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in epr.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeEPRAmount:
            return QVariant(nAnonymizeEPRAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case PrivateSendRounds:
            ...
            break;
        case AnonymizeEPRAmount:
            nAnonymizeEPRAmount = value.toInt();
            settings.setValue("nAnonymizeEPRAmount", nAnonymizeEPRAmount);
            emit anonymizeEPRAmountChanged(nAnonymizeEPRAmount);
            break;
        case CoinControlFeatures:
