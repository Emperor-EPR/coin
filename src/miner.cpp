...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("EPRMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("EPRMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("EPRMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("epr-miner");
...
///////|| !masternodeSync.IsSynced()
        ...
        LogPrintf("Running EPRMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
