...
#if defined(NDEBUG)
#error "EPR cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
    	nSubsidy = 0 * COIN;
    } else if(nHeight < 2001 && nHeight > 0) {
    		nSubsidy = 1000 * COIN;
    }else {
    	a = (nHeight / 2000)*2 + 8 ;
		nSubsidy = a * COIN;
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (!fLiteMode) {
        if (masternodeSync.RequestedMasternodeAssets > KARMANODE_SYNC_LIST) {
