...
pkgdatadir = $(datadir)/epr
pkgincludedir = $(includedir)/epr
pkglibdir = $(libdir)/epr
pkglibexecdir = $(libexecdir)/epr
...
CONFIG_HEADER = $(top_builddir)/src/config/epr-config.h
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	...
	$(top_srcdir)/src/config/epr-config.h.in \
...
ACLOCAL = ${SHELL} /root/emporer/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /root/emporer/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /root/emporer/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /root/emporer/build-aux/missing automake-1.15
AWK = gawk
BDB_CPPFLAGS = 
...
LUPDATE = 
MAINT = 
MAKEINFO = ${SHELL} /root/emporer/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL64 = 
PACKAGE = epr
PACKAGE_BUGREPORT = www.epr.org
PACKAGE_NAME = EPR Core
PACKAGE_STRING = EPR Core 2.3.0
PACKAGE_TARNAME = epr
PACKAGE_URL = 
...
ZMQ_CFLAGS = 
ZMQ_LIBS = 
abs_builddir = /root/emporer
abs_srcdir = /root/emporer
abs_top_builddir = /root/emporer
abs_top_srcdir = /root/emporer
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /root/emporer/build-aux/install-sh
...
SUBDIRS = src
GZIP_ENV = "-9n"
BITCOIND_BIN = $(top_builddir)/src/eprd$(EXEEXT)
BITCOIN_QT_BIN = $(top_builddir)/src/qt/epr-qt$(EXEEXT)
BITCOIN_CLI_BIN = $(top_builddir)/src/epr-cli$(EXEEXT)
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info block_test.info \
  leveldb_baseline.info test_epr_filtered.info total_coverage.info \
  baseline_filtered.info block_test_filtered.info \
  leveldb_baseline_filtered.info test_epr_coverage.info test_epr.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/EPR-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/epr-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/epr-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/epr-config.h
$(top_srcdir)/src/config/epr-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/epr-config.h src/config/stamp-h1
...
$(OSX_APP)/Contents/MacOS/EPR-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/EPR-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/EPR-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_epr.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_epr -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_epr_filtered.info: test_epr.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#block_test.info: test_epr_filtered.info
#	$(MKDIR_P) qa/tmp
...
#test_epr_coverage.info: baseline_filtered_combined.info test_epr_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_epr_filtered.info -o $@

#total_coverage.info:  baseline_filtered_combined.info test_epr_filtered.info block_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_epr_filtered.info -a block_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_epr.coverage/.dirstamp:  test_epr_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_epr.coverage/.dirstamp total.coverage/.dirstamp

# EPR: disable the outdated comparison tool test
...
clean-local:
	rm -rf test_epr.coverage/ total.coverage/ $(OSX_APP)
