Bitcoin Core version 0.14.0 is now available from:
  ...
  <https://epr.org/bin/epr-core-0.14.0/>
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/epr/epr/blob/master/doc/zmq.md) for
...
Commands sent over the JSON-RPC interface and through the `epr-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`epr-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/epr-cli -named help command="help"
    src/epr-cli -named getblockhash height=0
    src/epr-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/epr-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://epr.org/en/alert/2016-11-01-alert-retirement) in Bitcoin Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   ...
   optimizations to memory management. See [Pull #8753](https://github.com/epr/epr/pull/8753)
...
- #8722 `bb843ad` epr-cli: More detailed error reporting (laanwj)
...
- #8830 `ef0801b` test: Add option to run epr-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to epr-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to epr-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and epr-util-test.py (jnewbery)
...
- #9691 `fc67cd2` Init ECC context for `test_epr_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change epr address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` eprd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow epr-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` epr-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to epr-tx (jnewbery)
