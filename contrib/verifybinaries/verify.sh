...
###   eprcore.org and epr.org and compares them.
...
WORKINGDIR="/tmp/epr_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://eprcore.org"
HOST2="https://epr.org"
BASEDIR="/bin/"
VERSIONPREFIX="epr-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'epr-',
...
if [ $? -ne 0 ]; then
   echo "epr.org failed to provide signature file, but eprcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "epr.org and eprcore.org signature files were not equal?"
